...
begin
  ...
  -- in Supabase by default — the probes below query property.properties directly, relying
  -- on the table's own RLS policy, which postgres would otherwise skip entirely regardless
  -- of request.jwt.claims. Switching to authenticated (rolbypassrls = false) is what makes
  -- this a real behavioural proof rather than a check that always passes.
  --
  ...
  execute 'grant select on property.properties to authenticated';

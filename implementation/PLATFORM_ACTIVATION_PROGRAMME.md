...
| 7 | Mobile, accessibility, localization, error-state quality | Every new screen this session (`ServiceLocationField`, the disclosure-consent card) built and live-tested at the mobile viewport (375×812); its own address inputs got real `aria-label`s (PR #117). `docs/design/ACCESSIBILITY.md`'s own long-standing "Touch targets" finding closed as far as it honestly can be, 2026-08-28: `.sheet-close`/`.modal-close` fixed to the full 44px via hit-slop, live-verified with a real hit-test at a point outside the visible icon; `.photo-remove-btn` gets a smaller, deliberately partial 28px fix (its parent clips overflow, and the full 44px would turn a third of a small photo thumbnail into an invisible delete zone); `.chat-input-row button` (message send) turns out to be **genuinely un-fixable via hit-slop** — it lives inside a `Drawer`'s `overflow-y:auto` scroll container, and the CSS Overflow spec forces the other axis to compute as non-`visible` too, no matter what's declared, confirmed against the real computed style live, not just reasoned about. A real fix exists (restructure `Drawer`) but is out of this pass's scope, named honestly rather than left as silently-broken CSS. Chip touch-target sizing still deliberately not touched — shared primitive, much wider blast radius than these four already-diagnosed controls |

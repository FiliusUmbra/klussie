...
//
// WorkspaceSwitcher lives here, not only in AppShell's own topbar — that topbar is
// `display:none` below 460px (appStyles.js), which is every real phone (the app's actual,
// primary surface; the desktop-width phone mockup is the exception). A real professional
// always holds two live memberships (their personal workspace plus the one "become a pro"
// creates) and had no way to reach the second one at all on a real device before this —
// not a demo-only gap, a real dead end. Profile is where every other account-level action
// (edit profile, sign out) already lives; this is the same kind of action.

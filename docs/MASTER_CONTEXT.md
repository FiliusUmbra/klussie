...
Open from Epic 21     Live verification Pending. One diagnostic written
                       ...
                       data yet; every metric row is illustrative.
                       authenticated/anon hold no SELECT grant on either
                       store yet — ROLES.md §2.4's own "Not yet" bucket.
Open from Epic 20     Live verification Pending. One diagnostic written
                       across three migrations, none run. No live wiring
                       — nothing calls reindex_item() from another
                       engine's own event yet, so the index is empty.
                       authenticated/anon hold no SELECT grant on
                       derived.search_index yet — ROLES.md §2.4's own
                       "Not yet" bucket, opened by whichever epic ships
                       the live client read path.
...
| 🟢 Low | Literal `\uXXXX` escape text rendered in 2 places, down from 12 | Closed for every customer-reachable site (invoice totals, the quote-price prefix, the flexi tracker, the Boost price, the status-bar dots) — real `€`/`•` characters now, confirmed live on staging. The 2 remaining (`ServiceSheet.jsx`'s own en dash and middle dot) are dead, unreachable code (`CustomerApp.jsx`'s own header: `setActiveService` is never called anywhere), tied to a separate, already-partly-drafted dead-code removal, not a customer-visible defect | Close the remaining 2 alongside that dead-code removal (`Discover`/`ServiceSheet`/`QuoteFormSheet`'s own render path), not standalone | Phase 1 | P3 |
...
| ✅ Closed 2026-09-07 | `awaiting_pro` status leaks to the UI untranslated | ~~The status table in `src/lib/requestStatus.js` has no case for the status a directed request sits in (ADR-0012)~~ — corrected while closing this row: `awaiting_pro` is only ever written to the legacy `public.service_requests` table (`src/lib/requests.js`'s `createDirectedRequest()`); the correlated `work.requests` row — what `RequestsList`/`RequestDetailSheet` actually render, since WP 2.6's client cutover — is dual-written at plain `collecting` and structurally cannot hold `awaiting_pro` (`0182_service_location_schema.sql`'s own check constraint). No live customer ever saw the raw string; this row's original claim predated that cutover | Added a `statusAwaitingPro` key across all 10 locales and a `PRESENTATION` entry for it in `requestStatus.js` anyway — defensive coverage, not a live-bug close. `REQUEST_STATUS_ORDER`/the timeline deliberately does *not* gain an entry: a directed request's own `work.requests` row has no distinct lifecycle position to place one at. See `TESTING.md` §6.2 for the full trace. ADR-0012's own "Relief" promise (surfacing the directed pro's name, not just a generic status) remains a real, separate, unbuilt gap if directed requests become live traffic | Phase 1 | — |
...
| [0020](./adr/0020-events-partitioning-parameters.md) | Eight hash partitions, yearly time ranges, and a default range partition | **Proposed** — implemented against an empty table; free to revise until the first event is written |
| [0021](./adr/0021-one-audit-table-with-nullable-workspace.md) | One audit table with a nullable workspace, not two tables | **Proposed** — implemented against an empty table; free to revise until the first audit record is written |
| [0022](./adr/0022-backfilled-identifiers-are-uuidv7-minted-in-sql.md) | Backfilled identifiers are UUIDv7 minted in SQL from the row's own creation time | **Accepted** — governs every backfill in the roadmap; already load-bearing on staging's identity rows before acceptance |
| [0023](./adr/0023-identity-display-resolution-versus-row-visibility.md) | Cross-user profile reads resolve display information; they do not read the identity row | **Accepted** — implemented in WP 02.06 via resolvers in `public`; `identity` stays off the client API surface |
| [0024](./adr/0024-request-context-resolved-in-the-database.md) | Request context is resolved in the database until a gateway exists | **Accepted** — removal trigger is the first read needing capability or scope |
| [0025](./adr/0025-marketplace-visibility-survives-epic-03.md) | Pre-engagement marketplace visibility and public professional profiles survive Epic 03 unchanged | **Accepted** — WP 03.10 built against it exactly as written; removal trigger is Epic 12 |
| [0026](./adr/0026-membership-helper-lives-in-public.md) | The membership helper is a `SECURITY DEFINER` function in a dedicated `api` schema | **Accepted** — revised from an originally proposed placement in `public`; sets the pattern for every later engine's RLS helper |
| [0027](./adr/0027-workspace-permission-vocabulary.md) | The Workspace Permission Vocabulary | **Accepted** — twelve permissions scoped to workspace lifecycle and membership management only, deliberately excluding every other engine's future permissions |

...
- **Current milestone:** Epic 05 — Property Engine, **complete**
  2026-08-16 (6/6 packages). Epic 03 — Workspace Engine also complete
  2026-08-16 (12/12 packages) — the pivot of the roadmap. Neither
  verified against a live database (gate 10 open on both — see §12).
  Next: Epic 06 — Location Engine, or Epic 04 — Capability Engine. (§2)
...
```
Current Milestone     IMPLEMENTATION_ROADMAP.md Epic 05 — Property Engine
Status                COMPLETE (2026-08-16) — 6 of 6 work packages. Gate 10
                       (staging verification) open — see Open from Epic 05
                       below. Full record: implementation/epic-05/COMPLETION.md
Current Objective     Done: property.properties and .stewardship_periods exist;
                       every Personal Workspace stewards exactly one property
                       ("My Home"). Stewardship is a mutable current pointer
                       (property.properties.steward_workspace_id) plus a
                       genuinely append-only log of CLOSED periods only
                       (ADR-0028 — resolved a real contradiction between two
                       sections of DATABASE_ARCHITECTURE.md, found before any
                       code depended on the wrong reading). The isolation
                       policy and the client resolver both reuse Epic 03's
                       existing membership helper directly — no
                       property-specific resolver exists. The property engine
                       contract (my_properties, resolve_property) is built.
                       src/lib/homeInventory.js's fetchHomeProfile() resolves
                       the property's id and name; nothing downstream reads
                       them yet — the same "add without switching" restraint
                       Epic 03 WP09 held for workspace context.
Previous Milestone    Epic 03 — Workspace Engine (2026-08-16, complete, 12/12
                       packages) — the pivot of the roadmap. Record:
                       implementation/epic-03/COMPLETION.md
Current Branch        main (Epic 03 WP01–WP08 merged via PR #1 and #2); Epic
                       03 WP09–WP12 on one open branch/PR (#3), not yet
                       merged; Epic 05 (all 6 packages) not yet committed to
                       a branch
Next Deliverable      Epic 06 — Location Engine (depends on Epic 05, now
                       satisfied) or Epic 04 — Capability Engine (depends only
                       on Epic 03). Get a direct Postgres connection to
                       whichever session starts it — three epics' worth of
                       diagnostics are now unverified and a fourth would
                       compound a problem worth fixing once.
Open from Epic 05     Nothing in this epic has been run against any database —
                       new migrations 0039–0042, four new SQL diagnostics, all
                       written, none run, same connection gap as Epic 03.
                       Not yet pushed as a branch or opened as a PR, so no CI
                       run exists for it yet either (Epic 03 did get one, a
                       first for this roadmap).
...
| Testing | In Progress — **742 tests, 62 files**; all `src/lib` business logic, the homepage, both Property Memory surfaces, and Epics 01–05's schema/emission/consumer/workspace/property layers covered. SQL diagnostics verify the database posture against staging through Epic 03 WP08; WP 03.09 onward, and all of Epic 05, unrun (no DB connection this session). Every gate in Epics 01-05 was proven able to fail before being trusted — a discipline that found real defects in Epic 02 and a near-miss in Epic 03 (WP 03.11's public-profile reads). The feature components extracted from `App.jsx` (customer/pro/auth/profile) still have no render tests — their *rules* are tested, their markup is mostly not (WorkspaceSwitcher is the one exception, Epic 03) | Defined in Phase 2 | Improving | Unassigned |
...
| 🟠 High | **Epics 03 and 05 — nothing from WP 03.09 onward has been exercised against a live database** | Same class of gap as the Epic 02 row above, now three epics deep: no working credentials for either known test account, and no direct Postgres connection (pooler host/password, no linked Supabase CLI project) to run any of `VERIFY_WORKSPACE_ISOLATION_POLICIES.sql`, `VERIFY_LIST_MY_WORKSPACES.sql`, `VERIFY_PROPERTY.sql`, `VERIFY_BACKFILL_PROPERTY.sql`, `VERIFY_PROPERTY_CONTRACT.sql` or `VERIFY_PROPERTY_ISOLATION_POLICY.sql`. Covered by automated tests proving the exact fallback/switch logic instead | Get a working `.env.local` (pointed at staging, with valid seeded-account credentials) and a direct Postgres connection to this session, or the next one that continues Epic 06 — this has now compounded across three epics and is worth fixing once rather than carrying a fourth time | Epic 03, 05 | P1 |

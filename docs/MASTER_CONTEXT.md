...
| [0029](./adr/0029-client-access-pattern-for-new-engines.md) | RPC/API routes are the default client-access pattern for the new engines; direct PostgREST reads are the named exception | **Proposed** — governs Platform Activation Slice 1 onward; free to revise until the first Slice 1 client code lands |
| [0030](./adr/0030-operator-identity-via-operations-workspace.md) | Operator identity is a membership in a real, internal Operations Workspace — not a new access mechanism | **Proposed** — governs Platform Activation Slice 0 WP 0.3 onward; free to revise until the first real membership is seeded |

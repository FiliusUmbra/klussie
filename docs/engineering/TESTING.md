...
| C41 | ADR-0033 (2026-09-15, supersedes ADR-0007/0008) — My Home is its own bottom-nav destination now, not a segmented section of the Klussie tab; its own local My Home/My Items tab pair (moved out of ConversationHome.jsx unchanged) still holds every C23-C40 behavior above | `MyHomeScreen` *(automated)* |
| C42 | ADR-0033's own mockup — "Property health": real, derived data (open maintenance obligations, at least one overdue vs none) or nothing at all, never a fabricated score or percentage. Renders nothing when nothing has ever been tracked (`propertyHealthStatus()`, `src/lib/maintenance.js`) — the same "no claim without real data" restraint the trust strip already holds (ADR-0011) | `PropertyHealthCard` (`myHomeParts.jsx`) *(automated)* |
